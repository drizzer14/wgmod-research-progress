...
1. **Maxed-out elite vehicle rendering — RESOLVED: clamp.** Decided with the owner. Implemented in the domain layer (`builder._elite_ceiling` + clamp of `scale_min`/`fill_earned` to the cap; commit `b012d77`, test `test_elite_maxed_clamps_to_cap`). The bar renders **full** when maxed. The view (Task 12) should render `fill_earned` across `[scale_min, scale_max]` and treat a `scale_min == scale_max` range as 100%/complete (guard division-by-zero).
